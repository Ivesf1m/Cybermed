...
namespace mf
{
#define SCELL typename mfVertexStarIteratorQuadSurf<Traits>::sCell
#define IDS typename mfVertexStarIteratorQuadSurf<Traits>::ids

	template<class Traits>
	class mfVertexStarIteratorQuadSurf : public mfIterator<Traits>
	{
	public:
	...
	template<class Traits>
	mfVertexStarIteratorQuadSurf<Traits>::mfVertexStarIteratorQuadSurf(sMesh* _mesh)
	...
	template<class Traits>
	mfVertexStarIteratorQuadSurf<Traits>::~mfVertexStarIteratorQuadSurf()
	{

	}

	template<class Traits>
	bool mfVertexStarIteratorQuadSurf<Traits>::initialize(ids init)
	{
		assert(init >= 0);
		iv = init;
		v = mesh->getVertex(iv);
	...
	template<class Traits>
	bool mfVertexStarIteratorQuadSurf<Traits>::operator++()
	{
		if (status == 1) {
			if (currentIndex < 0) {
				if (is < 0) {
	...
	template<class Traits>
	void mfVertexStarIteratorQuadSurf<Traits>::addNeighbor()
	{
		ids temp;
	...
	template<class Traits>
	SCELL* mfVertexStarIteratorQuadSurf<Traits>::operator->()
	{
		return currentCell;
	}
	...
	template<class Traits>
	IDS mfVertexStarIteratorQuadSurf<Traits>::operator&()
	{
		return currentIndex;
	}

	template<class Traits>
	bool mfVertexStarIteratorQuadSurf<Traits>::finished()
	{
		return ((status == 2) || (status == 3));
	}

	template<class Traits>
	bool mfVertexStarIteratorQuadSurf<Traits>::notFinished()
	{
